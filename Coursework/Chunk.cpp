...
void Chunk::Initialise(int x, int y, int z)
{
	...
	Position_ = D3DXVECTOR3(x * Config::World::ChunkSize, y * Config::World::ChunkSize, z * Config::World::ChunkSize);
...
{
	// Delete the blocks
	for (int x = 0; x < Config::World::ChunkSize; x++)
	{
		// Loop through y dimension
		for (int y = 0; y < Config::World::ChunkSize; y++)
		{
			delete[] Blocks_[x][y];
		}
...
void Chunk::RefreshVisible()
{
	...
	for (int x = 0; x < Config::World::ChunkSize; x++)
	{
		...
		for (int y = 0; y < Config::World::ChunkSize; y++)
		{
			...
			for (int z = 0; z < Config::World::ChunkSize; z++)
			{
...
bool Chunk::Frame()
{
	...
	//if (distanceAway > 4 * Config::World::ChunkSize)
	//{
	//	IsVisible_ = false;
	//}
	//else
...
void Chunk::GenerateBlankChunk()
{
	...
	Blocks_ = new Block**[Config::World::ChunkSize];

	// Loop through x dimension
	for(int x = 0; x < Config::World::ChunkSize; x++)
	{
		Blocks_[x] = new Block*[Config::World::ChunkSize];

		// Loop through y dimension
		for (int y = 0; y < Config::World::ChunkSize; y++)
		{
			Blocks_[x][y] = new Block[Config::World::ChunkSize];
